 ...
 * @LastEditTime: 2022-07-18 20:36:36
 * @FilePath: /MyCProxy/lib/Util.cpp
 * @Descripttion: 
 */
#include <arpa/inet.h>
#include <errno.h>
...
int socketBindListen(int port) {
    ...
    // listen: 2048表示backlog，用于确定半连接和全连接队列的大小
...
// 忽略sigpipe信号，当收到rst包后，继续往关闭的socket中写数据，会触发sigpipe信号，默认会结束进程
void ignoreSigpipe() {
    struct sigaction sa;
    // 初始化变量
...
size_t readn(int fd, char *buffer, size_t size, bool &bufferEmpty) {
    ...
    while (len_left > 0) {
        ...
        if (len_left > 0) {
            // EINTR: 当阻塞某个慢系统调用的一个进程捕获某个信号且相应信号处理函数返回时，该系统调用可能返回一个EINTR错误
            // EAGAIN: 以O_NONBLOCK的标志打开文件/socket/FIFO时，如果连续做read操作而没有数据可读，此时程序不会阻塞等待数据准备就绪返回
            //          而是会返回一个EAGAIN错误
...
int tcp_connect(const char *host, u_int32_t server_port) {
    ...
    if (setfdNonBlock(sock_fd)) {
        SPDLOG_CRITICAL("set NONBLOCK err: {}", strerror(errno));
    }
    return sock_fd;
}
...
std::string rand_str(int len) {
    char tmp;       // 暂存一个随机数
    std::string buffer;     // 保存返回值
    std::random_device rd;   
    std::default_random_engine random(rd());    // 用rd初始化一个随机数发生器

    for (int i=0; i<len; i++) {
        tmp = random() % 36;    // 随机一个小于36的整数，0-9,A-Z共36种字符
        if (tmp < 10) {
            tmp += '0';
        } else {
            // 否则，变换为一个大写字母的ASCII
