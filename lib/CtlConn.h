 ...
 * @LastEditTime: 2022-07-18 20:35:47
 * @FilePath: /MyCProxy/lib/CtlConn.h
 * @Descripttion: 
 */
#pragma once
#include <memory>
...
public:
    ...
    CtlConn(int fd, SP_EventLoop loop)
        : Conn(fd, loop), out_buffer_(new Buffer(1024, 65535)), mutex_() {
    ...
    void setFreeProxyConnReqHandler_(MsgHandler handler) { 
        freeProxyConnReqHandler_ = handler;
    }
