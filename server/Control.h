/*
 * @Author: Jitong Zeng
 * @Date: 2022-07-21 19:43:51
 * @LastEditors: Jitong Zeng
 * @LastEditTime: 2022-07-21 20:12:36
 * @FilePath: /MyCProxy/server/Control.h
 * @Descripttion: 
 */
#pragma once
...
public:
    Control(int fd, std::string ctl_id, SP_EventLoop loop, Server* server)
        : ctl_conn_fd_(fd), ctl_id_(ctl_id), loop_(loop), server_(server) {
            ...
            conn_->setNewTunnelReqHandler(
                std::bind(&Control::handleNewTunnelReq, this, std::placeholders::_1, std::placeholders::_2)
            );
            conn_->setCloseHandler_(
                std::bind(&Control::handleCtlConnClose, this, std::placeholders::_1)
            );
            conn_->setNotifyProxyShutdownPeerConnHandler_(
                std::bind(&Control::handleShutdownPublicConn, this, std::placeholders::_1, std::placeholders::_2)
            );
            conn_->setFreeProxyConnReqHandler_(
                std::bind(&Control::handleFreeProxyConnReq, this, std::placeholders::_1, std::placeholders::_2)
            );
