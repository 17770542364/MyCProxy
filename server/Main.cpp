/*
 * @Author: Jitong Zeng
 * @Date: 2022-07-21 19:44:05
 * @LastEditors: Jitong Zeng
 * @LastEditTime: 2022-07-22 21:42:05
 * @FilePath: /MyCProxy/server/Main.cpp
 * @Descripttion: 
 */
#include <getopt.h>
#include <unistd.h>
...
{
    // 初始化日志格式
    spdlog::set_pattern("[%@ %H:%M:%S:%e %z] [%^%L%S] [thread %t] %v");
    int opt;
    int option_index = 0;
    ...
    printf("ctl_prot:%d; proxy_port:%d; work_thread_nums:%d\n", ctl_port, proxy_port, work_thread_nums);
