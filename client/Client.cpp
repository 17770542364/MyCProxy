 ...
 * @LastEditTime: 2022-07-19 20:55:02
 * @FilePath: /MyCProxy/client/Client.cpp
 * @Descripttion: 
 */
#include <assert.h>
...
void Client::initCtlConn() {
    ...
    ctl_conn_->setCloseHandler_(
        std::bind(&Client::handleCtlConnClose, this, std::placeholders::_1)
    );
    ...
    ctl_conn_->setNotifyProxyShutdownPeerConnHandler_(
        std::bind(&Client::handleShutdownLocalConn, this, std::placeholders::_1, std::placeholders::_2)
    );
}
...
void Client::handleNewTunnelRsp(void *msg, SP_CtlConn conn) {
    ...
    SPDLOG_CRITICAL("tunnel addr:{}{}", rsp_msg->proxy_server_host, rsp_msg->proxy_server_port);
...
void Client::handleProxyNotify(void *msg, SP_CtlConn conn) {
    ...
    // 将proxyConn设置为start，需要在send_msg_dirct之后，避免localConn先发送数据到Server端
...
void Client::handleShutdownLocalConn(void *msg, SP_CtlConn conn) {
    ...
    tun->shutdownLocalConn(proxyConn);
}
